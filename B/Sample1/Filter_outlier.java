...
public class Filter_outlier extends FilterFramework {
	...
	public void run()
    {
		...
		*************************************************************************************/
		...
		List<ArrayList<Long>> WaitingLIst=new ArrayList<ArrayList<Long>>();
					...
					if ((erg<0.0 || Math.abs(Double.longBitsToDouble(cash_Pressure)-erg)>10.0) &&cash_Pressure>0.0){  
						...
						if (count_outliers>0){
								 for (int ii = WaitingLIst.size()-count_outliers; ii < WaitingLIst.size(); ii++) {
										 WaitingLIst.get(ii).set(2, Double.doubleToRawLongBits((erg+ Double.longBitsToDouble(cash_Pressure))/2.0));
										 //writerb.print("\n");
	...
	for (ArrayList<Long> data_tupel : WaitingLIst){
		int idi=0;
