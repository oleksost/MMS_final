...
public class SystemA_SinkFilter extends FilterFramework
{
	...
	public SystemA_SinkFilter(String fileName){
	...
	public void run() 
    {
		...
		*************************************************************************************/
		...
		PrintWriter writer;
		try {
			//writer = new PrintWriter("/Users/d065820/Documents/HU_Belrin/MMS/MMS Abgabe/A/MMS_Shaldon/Output/OutputA.dat", "UTF-8");
			...
			while (true)
			{
				try
				{
					...
					****************************************************************************/
					...
					if ( id == 2 )
					{
						erg_id2 = Double.longBitsToDouble(measurement);
					} // if
					if ( id == 4 )
					{
						writer.format("%1s%15s%20s",TimeStampFormat.format(TimeStamp.getTime()), dft.format(Double.longBitsToDouble(measurement)), dfm.format(erg_id2));
						writer.print("\n");	
					} // if
