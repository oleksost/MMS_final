...
{
	public ArrayList<PipedInputStream> InputReadPorts = new ArrayList<PipedInputStream>();

    private PipedOutputStream[] OutputWritePorts = {new PipedOutputStream(),  new PipedOutputStream()};
...
public ArrayList<FilterFramework> InputFilters = new ArrayList<FilterFramework>();
	...
	*
	****************************************************************************/
	void Connect( FilterFramework[] FilterArray, int port )
	{
		for(FilterFramework Filter : FilterArray){
		try
		{
			// Connect this filter's input to the upstream pipe's output stream
			PipedInputStream InputReadPort = new PipedInputStream();
			InputReadPort.connect( Filter.OutputWritePorts[port]);
			InputReadPorts.add(InputReadPort);
	...
	****************************************************************************/
	...
	byte ReadFilterInputPort(PipedInputStream InputReadPort, FilterFramework InputFilter ) throws EndOfStreamException
	{
		byte datum = 0;
		...
		***********************************************************************/
		...
		try
		{
			...
			{
				if (EndOfInputStream(InputFilter))
				{
	...
	****************************************************************************/
	...
	void WriteFilterOutputPort_Double(byte[] dat){
		try
		{
			for (PipedOutputStream Port : OutputWritePorts){
				Port.write(dat);
